...
if IF_SMOTE
    ...
    SMOTE3 = smote(train_data(vector_train_label==3,:), 2, 2*100);
...
Last5Result = ResultList(end-4:end);
Specificity5=[];
Sensitivity5=[];
for i=1:5
    RRR  = Last5Result(i);
    Specificity5=[Specificity5;RRR.specificityList];
    Sensitivity5=[Sensitivity5;RRR.sensitivityList];
end
mean_sensitivity=mean(mean(Sensitivity5))*100
mean_Specificity = mean(mean(Specificity5))*100